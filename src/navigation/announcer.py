...
class AudioAnnouncer:
    ...
    def _worker(self):
        ...
        while self.running:
            try:
                # Clear queue if it's getting backed up (more than 2 messages)
                ...
                for prev_msg, prev_time in list(self.last_announced.items()):
                    ...
                    if time_since < config.MESSAGE_COOLDOWN:
                        ...
                        # Remove common words
                        common_words = {'on', 'your', 'and', 'the', 'a', 'an', 'in', 'front', '2', '3', '4', '5'}
                        msg_words -= common_words
                        prev_words -= common_words
                        
                        # If significant overlap (>60% of words), skip
                ...
                try:
                    ...
                    if self.use_espeak_direct:
                        # Use espeak directly via subprocess with shorter timeout
                        ...
                        if result.returncode != 0:
                            print(f"Warning: espeak failed with code {result.returncode}")
                    else:
                        # Use pyttsx3
                    ...
                    print(f"Warning: TTS timeout, killing process")
                    currently_speaking = False
                    # Clear any pending messages after timeout
                    while not self.message_queue.empty():
                        try:
                ...
                # Update timestamps only if speaking was successful
                if not currently_speaking:
                    self.last_announced[message] = current_time
                    self.last_global_announce = current_time
                
            except queue.Empty:
                # Clean up old entries from history periodically
                current_time = time.time()
                keys_to_remove = []
                for msg, timestamp in list(self.last_announced.items()):
                    if current_time - timestamp > config.MESSAGE_COOLDOWN * 2:
                        keys_to_remove.append(msg)
                for key in keys_to_remove:
                    del self.last_announced[key]
                continue
            except Exception as e:
