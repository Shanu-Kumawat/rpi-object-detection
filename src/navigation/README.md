# Zone-Based Object Detection with Audio Announcements

A real-time spatial awareness system using YOLO object detection with zone mapping and text-to-speech feedback. Ideal for accessibility applications, robotics, smart home systems, and assistive technology.

## Features

- **Real-time Object Detection**: Uses YOLOv11 (nano model) for fast, accurate object detection
- **Spatial Zone Mapping**: Divides camera view into three zones (left, center, right) for directional awareness
- **Audio Announcements**: Offline text-to-speech (TTS) provides clear, concise spoken feedback
- **Smart Filtering**: Temporal persistence filtering and throttling reduce false positives
- **Priority System**: Configurable priorities for different object classes
- **Extensible Design**: Ready for sensor fusion (ultrasonic, depth cameras, IMU, etc.)
- **Cross-platform**: Works on development machines and embedded devices (Raspberry Pi, etc.)

## Example Use Cases

- **Accessibility**: Navigation assistance for visually impaired users
- **Robotics**: Spatial awareness for autonomous navigation
- **Smart Home**: Object presence detection with location awareness
- **Security**: Intruder detection with zone-based alerts
- **Industrial**: Worker safety monitoring in designated zones
- **Education**: Interactive learning about computer vision and spatial awareness
